...
repeat:
	...
	jh_in->b_transaction != transaction) {
		...
		if (jh_in->b_orig == NULL) {
			if (jh_in->b_transaction)
				printk(KERN_ERR "%p... cur: %d/%d, commit: %d/%d\n", jh_in->b_orig, jh_in->b_transaction->t_journal->j_core_id, jh_in->b_transaction->t_tid, transaction->t_journal->j_core_id, transaction->t_tid);
			else
				printk(KERN_ERR "%p... NULL commit: %d/%d\n", jh_in->b_orig, transaction->t_journal->j_core_id, transaction->t_tid);
			printk(KERN_ERR "... tnext: %p cpnext: %p, shadow: %d, cpcount %d, jlist: %d\n", jh_in->b_tnext, jh_in->b_cpnext, buffer_shadow(bh_in), jh_in->b_cpcount, jh_in->b_jlist);
			free_out = 1;
			jbd_unlock_bh_state(bh_in);
			goto free;
		}
		...
		if (cold_jh->b_transaction != transaction) {
			printk(KERN_ERR "%p//tnext: %p cpnext: %p, shadow: %d, cpcount %d, jlist: %d\n",cold_jh, cold_jh->b_tnext, cold_jh->b_cpnext, buffer_shadow(new_bh), cold_jh->b_cpcount, cold_jh->b_jlist);
			printk(KERN_ERR "// cur: %d/%d, commit: %d/%d\n", cold_jh->b_transaction->t_journal->j_core_id, cold_jh->b_transaction->t_tid, transaction->t_journal->j_core_id, transaction->t_tid);

			if (jh_in->b_transaction)
				printk(KERN_ERR ".// cur: %d/%d, commit: %d/%d\n", jh_in->b_transaction->t_journal->j_core_id, jh_in->b_transaction->t_tid, transaction->t_journal->j_core_id, transaction->t_tid);
			else
				printk(KERN_ERR ".// NULL commit: %d/%d\n", transaction->t_journal->j_core_id, transaction->t_tid);
			printk(KERN_ERR "%p.//tnext: %p cpnext: %p, shadow: %d, cpcount %d, jlist: %d\n",jh_in, jh_in->b_tnext, jh_in->b_cpnext, buffer_shadow(bh_in), jh_in->b_cpcount, jh_in->b_jlist);
			free_out = 1;
			jbd_unlock_bh_state(bh_in);
			goto free;
		}
...
static int zj_seq_ov_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "  %u ms copy\n",
			jiffies_to_msecs(s->ovs->zj_copy_time));
	seq_printf(seq, "  %u ms wait\n",
			jiffies_to_msecs(s->ovs->zj_wait_time));
